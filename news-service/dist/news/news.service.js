...
let NewsService = class NewsService {
    ...
    async findAll(filterNewsDto) {
        const { searchTerm, publishedBefore, publishedAfter } = filterNewsDto;
        const query = this.newsRepository.createQueryBuilder('news');
        if (searchTerm) {
            query.andWhere('(news.title ILIKE :searchTerm OR news.shortDescription ILIKE :searchTerm)', { searchTerm: `%${searchTerm}%` });
        }
        if (publishedBefore) {
    ...
    async findOne(id) {
        const news = await this.newsRepository.findOneBy({ id, published: true });
        if (!news) {
            return {
                status: 'error',
